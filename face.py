...
def process_brand_type(brand_type):
    translation_map = {
        ...
        "Gesichtsserum": "Сыворотка для лица",
        "Augencreme": "Крем для кожи вокруг глаз",
        "Gesichtswasser": "Средство для умывания",
        "Wimpernserum": "Сыворотка для ресниц",
        "Gesichtsgel": "Гель для лица",
        "Nachtcreme": "Ночной крем",
        "Reinigungsöl": "Очищающее масло",
        "Gesichtspeeling": "Скраб для лица",
        "Zahnpasta": "Зубная паста",
        "Wimpernpflegeset": "Набор для ухода за ресницами"
    }
...
def parse_photos(driver):
    ...
    try:
        ...
        large_picture_element = driver.find_element(By.CSS_SELECTOR, 'picture.ProductPreviewSliderstyle__Picture-sc-195u70x-3 img')
        ...
        while True:
            try:
                image_elements = driver.find_elements(By.CSS_SELECTOR, 'picture.ProductPreviewSliderstyle__Picture-sc-195u70x-3 img')
...
async def main():
    try:
        ...
        # Начинаем с первой страницы каталога
        current_page_url = 'https://www.flaconi.at/gesicht/'
        ...
        while True:
            ...
            print(f"Найдено {len(product_links)} товаров на странице: {current_page_url}")
            ...
            for product_link in product_links:
                ...
                try:
                    brand_type_element = driver.find_element(By.CSS_SELECTOR, 'span[data-qa-block="product_brand_tyoe"]')
                    brand_type = brand_type_element.text.strip()
                    brand_type = process_brand_type(brand_type)
                except Exception:
                    brand_type = ""
                ...
                if photos:
                    ...
                    if sizes and prices:
                        data.append({
                            ...
                            'Price': '',
                            'Parent UID': '',
                            'Category': f"Уход за кожей лица;{brand_type};",
            ...
            # Перед поиском кнопки "Следующая страница" — дополнительный скролл вниз
            time.sleep(3)
            driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            time.sleep(2)

            # Поиск кнопки "Следующая страница" по XPath
            next_buttons = driver.find_elements(By.XPATH, "//a[contains(@class, 'Paginationstyle__NextPage-sc-d38xli-3')]")
            if next_buttons:
                next_href = next_buttons[0].get_attribute("href")
                print(f"Найден next_href: {next_href}")
                if next_href:
                    if next_href.startswith('/'):
                        current_page_url = f"https://www.flaconi.at{next_href}"
                    else:
                        current_page_url = next_href
                    print("Переход на следующую страницу:", current_page_url)
                    time.sleep(5)
                else:
        ...
        filename = f'Face_{current_time}.xlsx'
